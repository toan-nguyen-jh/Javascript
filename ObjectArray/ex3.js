...
class Questionaire {
  ...
  addQuestion(questionId, questionContent) {
    this.data[questionId] = questionContent;
  }

  editQuestion(questionId, questionContent) {
    const isQuestionExist = Object.keys(this.data).find(
      (id) => questionId === id
    );
    if (isQuestionExist) {
      this.data[questionId] = questionContent;
...
questionnaire.addQuestion("q7", {
  question: "Bạn còn muốn uống gì khác không?",
  options: { Có: "q8", Không: null },
});

questionnaire.editQuestion("q3", {
  question: "Bạn thích đồ uống gì khác?",
  options: { Trà: "q4", "Nước ép": "q7" },
});

questionnaire.deleteQuestion("q5");
